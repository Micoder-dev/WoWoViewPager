...
public class Easer implements TimeInterpolator {
    ...
    private static ArrayList<Easer> easers;

    public static Easer getInstance(Ease ease) {
        if (easers == null) {
            easers = new ArrayList<>(Ease.values().length);
            for (int length = Ease.values().length; length > 0; length--) easers.add(null);
        }
        Easer easer = easers.get(ease.getValue());
        if (easer == null) {
            easer = new Easer(ease);
            easers.set(ease.getValue(), easer);
        }
        return easer;
    }

    private Easer(Ease ease) {
        switch (ease) {
            case InBack:
